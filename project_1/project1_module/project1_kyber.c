...
static int team30_kyber_init_sched(struct request_queue *q, struct elevator_type *e)
{
	struct kyber_queue_data *kqd;
...
static void team30_kyber_exit_sched(struct elevator_queue *e)
{
...
static int team30_kyber_init_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team30_kyber_exit_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team30_kyber_limit_depth(unsigned int op, struct blk_mq_alloc_data *data)
{
	/*
...
static bool team30_kyber_bio_merge(struct blk_mq_hw_ctx *hctx, struct bio *bio,
...
static void team30_kyber_prepare_request(struct request *rq, struct bio *bio)
{
	rq_set_domain_token(rq, -1);
}

static void team30_kyber_insert_requests(struct blk_mq_hw_ctx *hctx,
...
static void team30_kyber_finish_request(struct request *rq)
{
...
static void team30_kyber_completed_request(struct request *rq, u64 now)
{
	struct kyber_queue_data *kqd = rq->q->elevator->elevator_data;
	struct kyber_cpu_latency *cpu_latency;
	unsigned int sched_domain;
...
static struct request *team30_kyber_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_queue_data *kqd = hctx->queue->elevator->elevator_data;
	struct kyber_hctx_data *khd = hctx->sched_data;
	struct request *rq;
	...
	spin_lock(&khd->lock);
	printk("In team30_kyber_dispatch_request function\n");
	/*
...
static bool team30_kyber_has_work(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_hctx_data *khd = hctx->sched_data;
	int i;
...
static ssize_t team30_kyber_##name##_lat_show(struct elevator_queue *e,	\
				       char *page)			\
{									\
	struct kyber_queue_data *kqd = e->elevator_data;		\
									\
	return sprintf(page, "%llu\n", kqd->latency_targets[domain]);	\
									...
									\
static ssize_t team30_kyber_##name##_lat_store(struct elevator_queue *e,	\
...
#define KYBER_LAT_ATTR(op) __ATTR(op##_lat_nsec, 0644, team30_kyber_##op##_lat_show, team30_kyber_##op##_lat_store)
...
static int team30_kyber_##name##_tokens_show(void *data, struct seq_file *m)	\
{									\
	...
	return 0;							\
									...
									\
static void *team30_kyber_##name##_rqs_start(struct seq_file *m, loff_t *pos)	\
	__acquires(&khd->lock)						\
{									\
	...
	return seq_list_start(&khd->rqs[domain], *pos);			\
									...
									\
static void *team30_kyber_##name##_rqs_next(struct seq_file *m, void *v,	\
				     loff_t *pos)			\
{									\
	struct blk_mq_hw_ctx *hctx = m->private;			\
	struct kyber_hctx_data *khd = hctx->sched_data;			\
	...
	return seq_list_next(v, &khd->rqs[domain], pos);		\
									...
									\
static void team30_kyber_##name##_rqs_stop(struct seq_file *m, void *v)	\
	__releases(&khd->lock)						\
{									\
	struct blk_mq_hw_ctx *hctx = m->private;			\
	struct kyber_hctx_data *khd = hctx->sched_data;			\
...
static const struct seq_operations kyber_##name##_rqs_seq_ops = {	\
	.start	= team30_kyber_##name##_rqs_start,				\
	.next	= team30_kyber_##name##_rqs_next,				\
	.stop	= team30_kyber_##name##_rqs_stop,				\
	.show	= blk_mq_debugfs_rq_show,				\
};									\
									\
static int team30_kyber_##name##_waiting_show(void *data, struct seq_file *m)	\
{									\
...
#define KYBER_QUEUE_DOMAIN_ATTRS(name)	\
	{#name "_tokens", 0400, team30_kyber_##name##_tokens_show}
...
#define KYBER_HCTX_DOMAIN_ATTRS(name)					\
	...
	{#name "_waiting", 0400, team30_kyber_##name##_waiting_show}
...
static struct elevator_type team30_kyber_sched = {
	.ops = {
		.init_sched = team30_kyber_init_sched,
		.exit_sched = team30_kyber_exit_sched,
		.init_hctx = team30_kyber_init_hctx,
		.exit_hctx = team30_kyber_exit_hctx,
		.limit_depth = team30_kyber_limit_depth,
		.bio_merge = team30_kyber_bio_merge,
		.prepare_request = team30_kyber_prepare_request,
		.insert_requests = team30_kyber_insert_requests,
		.finish_request = team30_kyber_finish_request,
		.requeue_request = team30_kyber_finish_request,
		.completed_request = team30_kyber_completed_request,
		.dispatch_request = team30_kyber_dispatch_request,
		.has_work = team30_kyber_has_work,
	},
#ifdef CONFIG_BLK_DEBUG_FS
	...
	.elevator_name = "team30",
	.elevator_owner = THIS_MODULE,
};

static int __init team30_kyber_init(void)
{
	return elv_register(&team30_kyber_sched);
}

static void __exit team30_kyber_exit(void)
{
	elv_unregister(&team30_kyber_sched);
}

module_init(team30_kyber_init);
module_exit(team30_kyber_exit);

MODULE_AUTHOR("team30");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("team30_Kyber I/O scheduler");
